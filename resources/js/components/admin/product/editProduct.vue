            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col border p-2">
                    ...
                    >
                        ...
                        <!-- دکمه حذف -->
                        <a
                            @click="extraFields.splice(index, 1)"
...
const fetchProduct=async ()=>{
    ...
    selectedCategory.value=product.value.category_id
    prodcutdesription.value=product.value.description
    productName.value=product.value.name
    productSlug.value=product.value.slug
    productStock.value=product.value.stock
    productIsActive.value=!!product.value.is_active

    productBrand.value=product.value.brand
    ...
    const imagesObj = JSON.parse(product.value.images)

    mainImagePreview.value = `/storage/images/products/${imagesObj.main}`
    mainImage.value=await fetchImageFromServer(mainImagePreview.value)

    thumbnailPreview.value=`/storage/images/products/thumb/${imagesObj.thumb}`
    thumbnail.value=await fetchImageFromServer(thumbnailPreview.value)
    await loadGalleryImages(imagesObj.gallery)
...
const loadGalleryImages = async (galleryList) => {
    for (const item of galleryList) {
        ...
        // پیش‌نمایش
        galleryPreviews.value.push(url)

        // دانلود و تبدیل به File
        const file = await fetchImageFromServer(url)
        if (file) {
...
const fetchImageFromServer = async (url) => {
    try {
        ...
        console.error('خطا در دریافت عکس:', error)
        return null
    }
}
onMounted(()=>{
...
const handleCategoryChange = async (category) => {
    ...
    if(!firstTimeLoad) {
       ...
       return
        ...
        extraFields.splice(0, extraFields.length)
