...
@foreach($bannerPostId->postmedia as $postmedia)

    @if($postmedia->type == 'banner')

        @php
            $bannerPath = $postmedia->file_path;
...
<div class="w-full md:w-{{$bannerwitdth}}/4 sm:w-1/2 flex flex-col h-[280px] rounded text-white bg-cover bg-center justify-between transition hover:brightness-75 duration-500 relative"
     style="background-image: linear-gradient(rgba(0, 0, 0, 0.05), rgba(0, 0, 0, 0.5)), url({{ asset('storage/images/blog/'.$bannerPostId->slug.'/'.$bannerPath) }})">
